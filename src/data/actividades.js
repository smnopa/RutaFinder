export const actividades = [
  {
    nombre: "Cartagena",
    actividades: [
      {
        id: "actividad1",
        nombre: "Visita al Castillo de San Felipe",
        img: "/img/Actividades.jpg",
        descripcion:
          "Un recorrido histórico por el famoso castillo de Cartagena.",
          bento: [
            "/img/hotelCaribe.jpg",
            "/img/hotelCaribe2.jpg",
            "/img/hotelCaribe3.jpg",
            "/img/hotelCaribe4.jpg",
            "/img/hotelCaribe5.jpg",
          ],
          descripciongeneral: [
            "Cancelación gratuita",
            "Tour y caminata",
            "Estadía 10.5 horas (con capacidad de extención)",
            "Servicio de recogida opcional",
          ],
          descripciondetalles: [
            "Cancela con hasta 24 horas de antelación y recibe un reembolso completo.",
            "Planes flexibles: reserva tu plaza de inmediato, sin que se te haga el cargo.",
            "Comprueba la disponibilidad para ver los horarios de inicio.",
            "Incluye el servicio de recogida en tu hotel en la zona de El Poblado. El tiempo máximo de espera en el hotel es de 5 minutos.",
          ],
          mapa: "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3916.4834565845715!2d-75.55579808471768!3d10.398763292568846!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x8ef62e5cbec569b7%3A0x104f74f7c27e5db0!2sHotel%20Caribe%20By%20Faranda%20Grand!5e0!3m2!1sen!2sco!4v1701183437394!5m2!1sen!2scohttps://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3916.4834565845715!2d-75.55579808471768!3d10.398763292568846!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x8ef62e5cbec569b7%3A0x104f74f7c27e5db0!2sHotel%20Caribe%20By%20Faranda%20Grand!5e0!3m2!1sen!2sco!4v1701183437394!5m2!1sen!2scohttps://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3916.726057678473!2d-75.531659!3d10.403060!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x8ef62e4e2a1a61f3%3A0x5d08edb25c0bc177!2sCra.%201%20%232–87%2C%20Cartagena%20de%20Indias%2C%20Provincia%20de%20Cartagena%2C%20Bol%C3%ADvar!5e0!3m2!1sen!2sco!4v1701184871450!5m2!1sen!2sco",
          contentData: [
            {
              titulo: "Historia y Cultura del Hotel Caribe en Cartagena Colombia",
              description:
                "El Hotel Caribe en Cartagena es un hito histórico y cultural que refleja la esencia de la ciudad. A continuacion te presentamos una breve descripción de su historia, cultura y algunos datos curiosos:",
              historia: {
                title: "Historia",
                content:
                  "Inaugurado en 1945, el Hotel Caribe fue uno de los primeros hoteles de lujo en Cartagena, diseñado para atraer tanto a turistas nacionales como internacionales. A lo largo de los años, el hotel ha sido testigo de la evolución de la ciudad y ha hospedado a importantes personalidades de la política, el entretenimiento y los negocios. Su arquitectura, que mezcla el estilo colonial con elementos modernos, ha sido restaurada y mantenida para preservar su encanto histórico. Durante más de 70 años, el Hotel Caribe ha sido un símbolo de hospitalidad y lujo en la costa caribeña colombiana.",
              },
              cultura: {
                title: "Cultura",
                content:
                  "El Hotel Caribe es parte integral de la cultura cartagenera, no solo por su historia como uno de los primeros grandes hoteles de la ciudad, sino también por su vinculación con eventos importantes. A lo largo de los años, ha sido escenario de encuentros sociales, empresariales y culturales, promoviendo la difusión del arte, la música y la gastronomía de la región. Además, su cercanía al centro histórico de Cartagena permite a los huéspedes sumergirse en la vibrante cultura local, que abarca desde su arquitectura colonial hasta sus tradiciones gastronómicas.",
              },
              mitos: {
                title: "Datos Curiosos",
                content:
                  "El Hotel Caribe se distingue por su ubicación privilegiada en el barrio de Bocagrande, una de las zonas más exclusivas y turísticas de Cartagena, conocida por sus playas y ambiente moderno. Su arquitectura histórica, que combina elementos coloniales con detalles contemporáneos, ha sido cuidadosamente preservada. Además, ofrece acceso directo a la playa, permitiendo a los huéspedes disfrutar del mar Caribe sin salir del hotel. A lo largo de los años, el hotel ha acogido a celebridades, políticos y artistas internacionales, convirtiéndose en un referente de lujo y hospitalidad en la ciudad. Su oferta gastronómica también es de renombre, con restaurantes que destacan los sabores del Caribe colombiano.",
              },
              images: [
                "/img/hotelCaribeH1.webp",
                "/img/hotelCaribeH2.webp",
                "/img/hotelCaribeH3.webp",
              ],
            },
          ],
          flyer: "/img/FlyerHotelCaribe.jpg",
      },
      {
        id: "actividad2",
        nombre: "Tour en Isla Baru",
        img: "/img/Actividades.jpg",
        descripcion: "Un día completo de sol y playa en Isla Baru.",
      },
    ],
  },
  {
    nombre: "Bogotá",
    actividades: [
      {
        id: "actividad3",
        nombre: "Paseo por la Candelaria",
        img: "/img/Actividades.jpg",
        descripcion:
          "Recorrido por el histórico barrio de La Candelaria en Bogotá.",
      },
      {
        id: "actividad4",
        nombre: "Museo del Oro",
        img: "/img/Actividades.jpg",
        descripcion:
          "Explora la fascinante historia del oro en el museo más grande del país.",
      },
    ],
  },
  {
    nombre: "Armenia y Salento",
    actividades: [
      {
        id: "Mirador-Colina-Iluminada",
        nombre: "Mirador Colina Iluminada",
        img: "/img/actiMirador.jpg",
        descripcion:
          "Estructura en madera icónica que tiene buena altura y te permite explorar la vista a 360° una aventura chévere que sería bueno repetir.",
          bento: [
            "/img/actiMirador1.jpg",
            "/img/actiMirador2.jpg",
            "/img/actiMirador3.jpg",
            "/img/actiMirador4.jpg",
            "/img/actiMirador5.jpg",
          ],
          descripciongeneral: [
            "Cancelación gratuita",
            "Reservar ahora y pagar después",
            "Estadía 10.5 horas (con capacidad de extención)",
            "Wi-Fi gratis",
            "Servicio de recogida opcional",
          ],
          descripciondetalles: [
            "Cancela con hasta 24 horas de antelación y recibe un reembolso completo.",
            "Planes flexibles: reserva tu plaza de inmediato, sin que se te haga el cargo.",
            "Comprueba la disponibilidad para ver los horarios de inicio.",
            "Red Wi-Fi para todo el hotel.",
            "Incluye el servicio de recogida en tu hotel en la zona de El Poblado. El tiempo máximo de espera en el hotel es de 5 minutos.",
          ],
          mapa: "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d318.09522404758444!2d-75.6685049!3d4.6689425!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x8e3889788ac94a81%3A0x7652446180ef8e53!2sMirador%20Colina%20Iluminada!5e0!3m2!1ses!2sco!4v1701184871450!5m2!1ses!2sco",
          contentData: [
            {
              titulo: "Historia y Cultura del Mirador Colina Iluminada en Filandia, Quindío, Colombia",
              description:
                "El Mirador Colina Iluminada en Filandia, Quindío, es uno de los principales atractivos turísticos de esta región. Filandia es conocida como la Hija de los Andes y fue fundada en 1878.",
              historia: {
                title: "Historia",
                content:
                  "El mirador está ubicado en el punto más alto de Filandia, ofreciendo una vista impresionante de la región, incluyendo el Valle del Cocora, las montañas andinas y parte del paisaje cafetero, que ha sido declarado Patrimonio de la Humanidad por la UNESCO. Este mirador es relativamente nuevo, ya que se construyó como parte de los esfuerzos de la región para potenciar el turismo sostenible y mostrar el esplendor del paisaje natural que rodea a Filandia.",
              },
              cultura: {
                title: "Cultura",
                content:
                  "La cultura del Eje Cafetero, y en especial de Filandia, está profundamente enraizada en la producción de café. Desde el mirador, los visitantes pueden contemplar el vasto paisaje cafetero, que es una parte fundamental de la identidad de la región. Además, la arquitectura del mirador tiene influencia en la estética rural, reflejando la conexión entre la tradición y el desarrollo turístico. La comunidad local celebra eventos culturales, especialmente en torno al café y las tradiciones campesinas, lo que permite a los visitantes sumergirse en la cultura cafetera de la zona.",
              },
              mitos: {
                title: "Datos Curiosos",
                content:
                  "El mirador tiene una estructura de aproximadamente 27 metros de altura y cuenta con un diseño único, que permite ver hasta 360 grados del paisaje, desde la cima del mirador se pueden observar hasta 11 municipios diferentes del Quindío y Risaralda en días despejados, además este lugar también es conocido por sus espectaculares puestas de sol y es un punto ideal para avistar aves, ya que Filandia está rodeada de reservas naturales que son hogar de diversas especies.",
              },
              images: [
                "/img/hycMirador1.jpg",
                "/img/hycMirador2.jpg",
                "/img/hycMirador3.jpg",              
              ],
            },
          ],
          flyer: "/img/fliyermirador.jpg",
        },
      {
        id: "Las-Acacias-Coffee",
        nombre: "Las Acacias Coffee Farm",
        img: "/img/actiAcacias.jpg",
        descripcion:
          "Finca cafetera tradicional colombiana. Mostramos a los turistas cómo se hace el mejor café del mundo.",
          bento: [
            "/img/actiAcacias1.jpg",
            "/img/actiAcacias2.jpg",
            "/img/actiAcacias3.jpg",
            "/img/actiAcacias4.jpg",
            "/img/actiAcacias5.jpeg",
          ],
          descripciongeneral: [
            "Cancelación gratuita",
            "Reservar ahora y pagar después",
            "Estadía 10.5 horas (con capacidad de extención)",
            "Enseñanza y muestra de café",
            "Servicio de recogida opcional",
          ],
          descripciondetalles: [
            "Cancela con hasta 24 horas de antelación y recibe un reembolso completo.",
            "Planes flexibles: reserva tu plaza de inmediato, sin que se te haga el cargo.",
            "Comprueba la disponibilidad para ver los horarios de inicio.",
            "Ofrece la experiencia de un caficultor local y profesional.",
            "Incluye el servicio de recogida en tu hotel en la zona de El Poblado. El tiempo máximo de espera en el hotel es de 5 minutos.",
          ],
          mapa: "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d318.09522404758444!2d-75.6685049!3d4.6689425!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x8e3889788ac94a81%3A0x7652446180ef8e53!2sMirador%20Colina%20Iluminada!5e0!3m2!1ses!2sco!4v1701184871450!5m2!1ses!2sco",
          contentData: [
            {
              titulo: "Historia y Cultura de Las Acacias Coffee Farm en Colombia",
              description:
                "Las Acacias Coffee Farm, ubicada en la región cafetera de Salento, Quindío, forma parte de la rica tradición cafetera del Eje Cafetero, Patrimonio de la Humanidad de la UNESCO.",
              historia: {
                title: "Historia",
                content:
                  "Fundada como una finca cafetera tradicional, es un espacio donde los visitantes pueden sumergirse en la historia del cultivo de café en Colombia. En esta finca, se preservan las prácticas ancestrales del cultivo y procesamiento del café, transmitidas de generación en generación. Los caficultores locales desempeñan un papel vital como guardianes de esta tradición, compartiendo con los turistas los secretos del cultivo del grano que ha definido la identidad cultural y económica de la región durante décadas",
              },
              cultura: {
                title: "Cultura",
                content:
                  "La cultura de Las Acacias está profundamente enraizada en la pasión por el café. Aquí, el proceso del café es más que una actividad agrícola: es una expresión cultural y una forma de vida. Los caficultores no solo producen café, sino que también contribuyen al desarrollo de un turismo sostenible que destaca la biodiversidad del paisaje, la riqueza cultural del Quindío y la importancia del respeto por el medio ambiente. La finca ofrece una experiencia única, permitiendo a los visitantes participar activamente en la recolección y el tostado de los granos, mientras aprenden sobre las técnicas artesanales y la historia del café colombiano",
              },
              mitos: {
                title: "Datos Curiosos",
                content:
                  "En Las Acacias, los visitantes no solo aprenden sobre el café, sino que pueden experimentar el proceso completo, desde la recolección hasta la taza, lo que convierte la visita en una inmersión cultural y sensorial, tambien la finca promueve prácticas sostenibles y ecológicas, enfocándose en la conservación del entorno natural y el uso responsable de los recursos, además Los turistas tienen la oportunidad de degustar diferentes tipos de cafés especiales, cada uno con perfiles de sabor únicos que van desde notas frutales hasta tonos especiados y florales, lo que refleja la complejidad de los granos producidos en esta región​",
              },
              images: [
                "/img/hycAcacias1.jpg",
                "/img/hycAcacias2.jpeg",
                "/img/hycAcacias3.jpg",              
              ],
            },
          ],
          flyer: "/img/flyerAcacias.webp",
      },
      {
        id: "Bosques-Cocora-Juan",
        nombre: "Bosques de Cocora Donde Juan",
        img: "/img/actiBosques.jpg",
        descripcion:
          "Finca cafetera tradicional colombiana. Mostramos a los turistas cómo se hace el mejor café del mundo.",
          bento: [
            "/img/actiBosques5.jpg",
            "/img/actiBosques2.jpg",
            "/img/actiBosques3.jpg",
            "/img/actiBosques4.jpg",
            "/img/actiBosques1.jpg",
          ],
          descripciongeneral: [
            "Cancelación gratuita",
            "Reservar ahora y pagar después",
            "Estadía 10.5 horas (con capacidad de extención)",
            "Caminata por los Bosques de Cocora",
            "Servicio de recogida opcional",
          ],
          descripciondetalles: [
            "Cancela con hasta 24 horas de antelación y recibe un reembolso completo.",
            "Planes flexibles: reserva tu plaza de inmediato, sin que se te haga el cargo.",
            "Comprueba la disponibilidad para ver los horarios de inicio.",
            "Camina al lado de un guia y disfruta de la hermosa vista de los bosques de Cocora Donde Juan.",
            "Incluye el servicio de recogida en tu hotel en la zona de El Poblado. El tiempo máximo de espera en el hotel es de 5 minutos.",
          ],
          mapa: "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d318.0954913912617!2d-75.4903831!3d4.6379647!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x8e388db9a2b8e3bf%3A0x21ee961d720791e7!2sDonde%20Juan%20B%20Bosques%20de%20Cocora%20Valle%20De%20cocora!5e0!3m2!1ses!2sco!4v1701184871450!5m2!1ses!2sco",
          contentData: [
            {
              titulo: "Historia y Cultura de Los Bosques de Cocora Donde Juan en Colombia",
              description:
                "El restaurante y hospedaje Bosques de Cocora Donde Juan B está ubicado en el hermoso Valle de Cocora, cerca de Salento, Quindío, un lugar reconocido por sus majestuosas palmas de cera, el árbol nacional de Colombia. Este establecimiento combina aventura, naturaleza y gastronomía auténtica, ofreciendo una experiencia completamente inmersa en la biodiversidad y cultura del Eje Cafetero.",
              historia: {
                title: "Historia",
                content:
                  "Este lugar ha sido parte del desarrollo turístico del Valle de Cocora, una región históricamente dedicada a la conservación del medio ambiente y al turismo ecológico. Las caminatas guiadas por el bosque nuboso y la interacción con la naturaleza son una parte central de la oferta cultural, permitiendo a los visitantes conocer de cerca la flora y fauna locales, con una atención especial en la conservación de las palmas de cera y la diversidad de aves exóticas como los colibríes.",
              },
              cultura: {
                title: "Cultura",
                content:
                  "El enfoque de Bosques de Cocora Donde Juan B no es solo sobre el turismo, sino también sobre la sostenibilidad. El restaurante utiliza ingredientes frescos y orgánicos cultivados en su propia huerta, promoviendo una conexión profunda con la tierra y la gastronomía local. Los platos reflejan la cultura gastronómica del Quindío, con la trucha fresca como especialidad, un platillo típico de la región. Este respeto por los productos locales y su preparación sostenible representa la cultura agrícola de la zona y su compromiso con la preservación del entorno.",
              },
              mitos: {
                title: "Datos Curiosos",
                content:
                  "El restaurante se destaca por su compromiso con la gastronomía ecológica, y los vegetales utilizados provienen de una huerta orgánica propia, lo que refuerza el concepto de sostenibilidad, Además de sus deliciosos platos, se ofrecen experiencias personalizadas, como caminatas guiadas, observación de aves y talleres de inmersión cultural, lo que convierte al lugar en un punto de encuentro entre la naturaleza y la cultura local, Las palmas de cera, visibles desde el establecimiento, son las más altas del mundo, alcanzando hasta 60 metros y con una longevidad de 200 años​",
              },
              images: [
                "/img/hycBosques1.jpg",
                "/img/hycBosques2.jpg",
                "/img/hycBosques3.jpg",              
              ],
            },
          ],
          flyer: "/img/flyerBosques.jpg",
      },
      {
        id: "Ecoparque-Mirador.Alto-La-Cruz",
        nombre: "Ecoparque The Mirador Alto De La Cruz",
        img: "/img/actMirador.jpg",
        descripcion:
          "Excelente vista del pueblo del valle del cocora un clima delicioso y pasas por la calle real",
          bento: [
            "/img/actSalento4.jpg",
            "/img/actSalento1.jpg",
            "/img/actSalento2.jpg",
            "/img/actSalento3.jpg",
            "/img/actSalento5.webp",
          ],
          descripciongeneral: [
            "Cancelación gratuita",
            "Reservar ahora y pagar después",
            "Estadía 10.5 horas (con capacidad de extención)",
            "Caminata por el cendero real de Salento",
            "Servicio de recogida opcional",
          ],
          descripciondetalles: [
            "Cancela con hasta 24 horas de antelación y recibe un reembolso completo.",
            "Planes flexibles: reserva tu plaza de inmediato, sin que se te haga el cargo.",
            "Comprueba la disponibilidad para ver los horarios de inicio.",
            "Camina al lado de un guia y disfruta de las hermosas vistas del pueblo y sus alrededores.",
            "Incluye el servicio de recogida en tu hotel en la zona de El Poblado. El tiempo máximo de espera en el hotel es de 5 minutos.",
          ],
          mapa: "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d318.0952650271012!2d-75.5663879!3d4.6397978!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x8e388dc9da50482f%3A0x27e4c90e6e306735!2sMirador%20Alto%20de%20la%20Cruz!5e0!3m2!1ses!2sco!4v1701184871450!5m2!1ses!2sco",
          contentData: [
            {
              titulo: "Historia y Cultura del Ecoparque The Mirador Alto De La Cruz en Colombia",
              description:
                "El Ecoparque The Mirador Alto de la Cruz en Salento, Quindío, es uno de los puntos más visitados por los turistas en la región del Eje Cafetero, no solo por las espectaculares vistas que ofrece, sino también por su relevancia cultural y religiosa. Este mirador se encuentra en una colina a la cual se accede subiendo por la famosa Calle Real de Salento, un recorrido lleno de encanto y tradición que refleja la cultura cafetera de la zona.",
              historia: {
                title: "Historia",
                content:
                  "El mirador ha sido un punto de referencia tanto para locales como para visitantes desde hace varios años. Su acceso incluye un camino adornado con un via crucis, lo que añade un componente religioso y espiritual a la experiencia. Este sendero simbólico representa el calvario de Cristo hacia la cruz, lo cual convierte el lugar no solo en un atractivo turístico, sino también en un espacio de reflexión para quienes practican la fe católica.",
              },
              cultura: {
                title: "Cultura",
                content:
                  "La ubicación del Mirador Alto de la Cruz es clave dentro del Paisaje Cultural Cafetero, una región reconocida por la UNESCO como Patrimonio de la Humanidad. Este mirador permite disfrutar de las vastas vistas del Valle del Cocora, famoso por sus palmas de cera, el árbol nacional de Colombia. Además, en el parque se pueden encontrar vendedores de productos locales como caña de azúcar, obleas y artesanías, lo que fomenta la conexión con la economía y cultura local.",
              },
              mitos: {
                title: "Datos Curiosos",
                content:
                  "Las escaleras coloridas que conducen al mirador no solo son un atractivo visual, sino que se han convertido en un símbolo de Salento. Las personas que suben estas escaleras suelen tomarse fotos debido al contraste con el paisaje verde que las rodea, además Desde el mirador, los días despejados permiten ver más allá del Valle del Cocora, capturando impresionantes paisajes de la cordillera central y los ríos que cruzan la región, también un excelente punto para observar aves y, en ocasiones, se pueden ver cóndores de los Andes volando en los alrededores.​",
              },
              images: [
                "/img/hycSalento1.jpg",
                "/img/hycSalento2.jpg",
                "/img/hycSalento3.jpg",              
              ],
            },
          ],
          flyer: "/img/flyerSalento.jpg",
      },
      {
        id: "Jardín-Botánico-Quindío",
        nombre: "Jardín Botánico del Quindío",
        img: "/img/actiJardin.jpg",
        descripcion:
          "Si quieres visitar físicamente nuestra sede principal en Calarcá vivirás una experiencia inolvidable. El Jardín te espera.",
          bento: [
            "/img/actiJardin4.jpg",
            "/img/actiJardin1.jpg",
            "/img/actiJardin2.jpg",
            "/img/actiJardin3.jpg",
            "/img/actiJardin5.jpg",
          ],
          descripciongeneral: [
            "Cancelación gratuita",
            "Reservar ahora y pagar después",
            "Estadía 10.5 horas (con capacidad de extención)",
            "Mariposario",
            "Avistamiento de aves",
          ],
          descripciondetalles: [
            "Cancela con hasta 24 horas de antelación y recibe un reembolso completo.",
            "Planes flexibles: reserva tu plaza de inmediato, sin que se te haga el cargo.",
            "Comprueba la disponibilidad para ver los horarios de inicio.",
            "Descubre nuestro mariposario y disfruta de la experiencia de la naturaleza.",
            "El avistamiento de aves es una experiencia inolvidable en el Jardín Botánico del Quindío.",
          ],
          mapa: "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d318.09499354124655!2d-75.6535521!3d4.5116806!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x8e38f43b10d6dddd%3A0xf97d58b928ca87fe!2sJard%C3%ADn%20Bot%C3%A1nico%20Del%20Quind%C3%ADo!5e0!3m2!1ses!2sco!4v1701184871450!5m2!1ses!2sco",
          contentData: [
            {
              titulo: "Historia y Cultura del Jardín Botánico del Quindío en Colombia",
              description:
                "El Jardín Botánico del Quindío, ubicado en Calarcá, es un referente en biodiversidad y conservación ecológica en Colombia. Su historia comienza en 1978, cuando Alberto Gómez Mejía fundó este espacio con el objetivo de preservar las especies nativas del Quindío, incluyendo plantas endémicas, aves, e insectos. Desde entonces, ha crecido hasta convertirse en un importante centro de investigación y educación ambiental.",
              historia: {
                title: "Historia",
                content:
                  "El Jardín fue diseñado en terrenos que fueron devastados por un terremoto en 1999, y parte de su infraestructura, como la pérgola de hierro retorcido, simboliza la reconstrucción tras esa tragedia. También alberga varios museos que narran la evolución de la flora, fauna y geología de la región. Uno de los más interesantes es el Museo del Túnel, que explora la historia de los túneles viales construidos en los Andes del Quindío",
              },
              cultura: {
                title: "Cultura",
                content:
                  "Este espacio no solo destaca por su enfoque en la conservación, sino también por su profunda conexión con la cultura local. El museo etnobotánico presenta una colección de productos hechos de palmas nativas, como utensilios de cocina, muebles, y alimentos, que muestran cómo las comunidades han usado estas plantas durante siglos. Además, sus talleres ecológicos están orientados a jóvenes y niños para enseñar la importancia de la biodiversidad y la sostenibilidad",
              },
              mitos: {
                title: "Datos Curiosos",
                content:
                  "Uno de los aspectos más fascinantes del Jardín es su mariposario en forma de mariposa, una estructura icónica que permite a los visitantes observar de cerca estas coloridas especies. También cuenta con una colección de raíces de bambú que algunos visitantes encuentran similares a figuras alienígenasOtro dato interesante es el Insectario, donde se exponen especies endémicas y se promueve el estudio de los insectos como parte vital de los ecosistemas.​",
              },
              images: [
                "/img/hycjardin1.webp",
                "/img/hycjardin2.webp",
                "/img/hycjardin3.webp",              
              ],
            },
          ],
          flyer: "/img/flyerJardin.png",
      },
      {
        id: "Vida-Park-Quindío",
        nombre: "Vida Park Armenia Quindío",
        img: "/img/actiVidaPark.jpg",
        descripcion:
          "Un páramo en el medio de la ciudad que resulta un paso obligado para los amantes de la naturaleza en Armenia.",
          bento: [
            "/img/actiVidaPark1.jpg",
            "/img/actiVidaPark2.jpg",
            "/img/actiVidaPark3.webp",
            "/img/actiVidaPark4.jpg",
            "/img/actiVidaPark5.jpg",
          ],
          descripciongeneral: [
            "Cancelación gratuita",
            "Reservar ahora y pagar después",
            "Estadía 10.5 horas (con capacidad de extención)",
            "Tour por el parque",
            "Avistamiento de aves",
          ],
          descripciondetalles: [
            "Cancela con hasta 24 horas de antelación y recibe un reembolso completo.",
            "Planes flexibles: reserva tu plaza de inmediato, sin que se te haga el cargo.",
            "Comprueba la disponibilidad para ver los horarios de inicio.",
            "Camina y disfruta de un espacio tranquilo y natural, en medio de la ciudad",
            "El avistamiento de aves es una experiencia inolvidable.",
          ],
          mapa: "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d318.09469667956496!2d-75.6629152!3d4.5479794!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x8e38f4fb8f430bbd%3A0x159c0ed6d96d65f!2sParque%20De%20La%20Vida!5e0!3m2!1ses!2sco!4v1701184871450!5m2!1ses!2sco",
          contentData: [
            {
              titulo: "Historia y Cultura de Vida Park Armenia Colombia",
              description:
                "Vida Park es un importante espacio recreativo y natural ubicado en Armenia, Quindío, Colombia, en el corazón del Eje Cafetero",
              historia: {
                title: "Historia",
                content:
                  "Aunque no tiene una historia tan extensa como algunos monumentos históricos, su importancia radica en su papel como un punto de encuentro moderno para la recreación y el turismo, siendo un pulmón verde en medio de la ciudad. Es parte de los esfuerzos de urbanización sostenible en Armenia, con el objetivo de brindar a los residentes y turistas un lugar para disfrutar de la naturaleza y realizar actividades al aire libre.",
              },
              cultura: {
                title: "Cultura",
                content:
                  "El parque refleja la identidad cultural del Quindío, al estar rodeado de la tradición cafetera que define a la región. Armenia, como parte del Paisaje Cultural Cafetero, resalta su herencia a través de espacios como Vida Park, donde se realizan eventos culturales que celebran la biodiversidad, la ecología y el turismo sostenible. El parque también forma parte de los destinos frecuentados durante festividades locales, como el Festival de la Cosecha que celebra la cultura cafetera del Quindío​",
              },
              mitos: {
                title: "Datos Curiosos",
                content:
                  "Vida Park no solo es un espacio para la recreación, sino también para la educación ambiental. Ofrece actividades que buscan concientizar a los visitantes sobre la importancia de preservar el medio ambiente y la biodiversidad local, además El parque alberga una variedad de especies de flora y fauna, convirtiéndolo en un lugar de interés tanto para los amantes de la naturaleza como para quienes buscan un espacio tranquilo en medio de la ciudad, y tambien s un punto importante para actividades físicas como el ciclismo y el senderismo, lo que lo hace muy popular entre los deportistas locales y turistas.​",
              },
              images: [
                "/img/hycVidaPark1.jpg",
                "/img/hycVidaPark2.jpg",
                "/img/hycVidaPark3.jpg",
                              
              ],
            },
          ],
          flyer: "/img/flyerVida.jpg",
      },
    ],
  },
  {
    nombre: "Bogotá",
    actividades: [
      {
        id: "actividad3",
        nombre: "Paseo por la Candelaria",
        img: "/img/Actividades.jpg",
        descripcion:
          "Recorrido por el histórico barrio de La Candelaria en Bogotá.",
      },
      {
        id: "actividad4",
        nombre: "Museo del Oro",
        img: "/img/Actividades.jpg",
        descripcion:
          "Explora la fascinante historia del oro en el museo más grande del país.",
      },
    ],
  },
  {
    nombre: "Bogotá",
    actividades: [
      {
        id: "actividad3",
        nombre: "Paseo por la Candelaria",
        img: "/img/Actividades.jpg",
        descripcion:
          "Recorrido por el histórico barrio de La Candelaria en Bogotá.",
      },
      {
        id: "actividad4",
        nombre: "Museo del Oro",
        img: "/img/Actividades.jpg",
        descripcion:
          "Explora la fascinante historia del oro en el museo más grande del país.",
      },
    ],
  },
  {
    nombre: "Bogotá",
    actividades: [
      {
        id: "actividad3",
        nombre: "Paseo por la Candelaria",
        img: "/img/Actividades.jpg",
        descripcion:
          "Recorrido por el histórico barrio de La Candelaria en Bogotá.",
      },
      {
        id: "actividad4",
        nombre: "Museo del Oro",
        img: "/img/Actividades.jpg",
        descripcion:
          "Explora la fascinante historia del oro en el museo más grande del país.",
      },
    ],
  },
];
